...
export let config: { variantGroup: boolean, strictVariable: boolean, strictHyphen: boolean, useHex: boolean, classMode: boolean, presets: any[] } = {
  variantGroup: true,
  strictVariable: true,
  strictHyphen: false,
  useHex: false,
...
export function getConfig() {
  const { variantGroup, strictVariable, strictHyphen, useHex, classMode, presets = [] } = require('@vscode-use/utils').getConfiguration('unot')
  return {
    variantGroup,
    strictVariable,
    strictHyphen,
...
const COMMON_REG = config.strictHyphen
...
export function getRules() {
  return [
    ...
    [COMMON_REG, (_: string, prefix: string, v: string, v1 = '', v2 = '') => {
      ...
      if (v === 'text') {
        if (v2)
          return config.strictVariable ? `${prefix}${v}-[${v1}${v2}]` : `${prefix}${v}-${v1}${v2}`
    ...
    config.variantGroup
      ...
      : undefined,
    config.strictHyphen
      ? [/([\s'])(bg|text|border)-(\#[^\s']+)(\s|'|!|$)/g, (_: string, v: string, v1: string, v2: string, v3: string) => `${v}${v1}-${config.strictVariable ? '[' : ''}${v2}${config.strictVariable ? ']' : ''}${v3}`]
      : [/([\s'])(bg|text|border)-?(\#[^\s']+)(\s|'|!|$)/g, (_: string, v: string, v1: string, v2: string, v3: string) => `${v}${v1}-${config.strictVariable ? '[' : ''}${v2}${config.strictVariable ? ']' : ''}${v3}`],
    [/([\s'])border-box(\s|'|!|$)/, (_: string, v1 = '', v2: string) => `${v1}box-border${v2}`],
    [/([\s'])content-box(\s|'|!|$)/, (_: string, v1 = '', v2: string) => `${v1}box-content${v2}`],
    config.strictHyphen
      ? [/([A-Za-z]+)-\[?\s*(rgba?\([^\)]*\))\]?(\s|'|!|$)/g, (_: string, v0: string, v: string, v1: string) => `${v0}-${config.strictVariable ? '[' : ''}${v.replace(/\s*\/\s*/g, ',').replace(/\s+/g, ',').replace(/,+/g, ',')}${config.strictVariable ? ']' : ''}${v1}`]
      : [/([A-Za-z]+)-?\[?\s*(rgba?\([^\)]*\))\]?(\s|'|!|$)/g, (_: string, v0: string, v: string, v1: string) => `${v0}-${config.strictVariable ? '[' : ''}${v.replace(/\s*\/\s*/g, ',').replace(/\s+/g, ',').replace(/,+/g, ',')}${config.strictVariable ? ']' : ''}${v1}`],
    config.strictHyphen
      ? [/([A-Za-z]+)-\[?\s*(hsla?\([^\)]*\))\]?(\s|'|!|$)/g, (_: string, v0: string, v: string, v1: string) => `${v0}-${config.strictVariable ? '[' : ''}${v.replace(/\s*\/\s*/g, ',').replace(/\s+/g, ',').replace(/,+/g, ',')}${config.strictVariable ? ']' : ''}${v1}`]
      : [/([A-Za-z]+)-?\[?\s*(hsla?\([^\)]*\))\]?(\s|'|!|$)/g, (_: string, v0: string, v: string, v1: string) => `${v0}-${config.strictVariable ? '[' : ''}${v.replace(/\s*\/\s*/g, ',').replace(/\s+/g, ',').replace(/,+/g, ',')}${config.strictVariable ? ']' : ''}${v1}`],
    config.strictHyphen
      ? [/([A-Za-z]+)-\[?\s*(calc\([^\)]*\))\]?(\s|'|!|$)/g, (_: string, v0: string, v: string, v1 = '') => `${v0}-${config.strictVariable ? '[' : ''}${v.replace(/\s*/g, '')}${config.strictVariable ? ']' : ''}${v1}`]
      : [/([A-Za-z]+)-?\[?\s*(calc\([^\)]*\))\]?(\s|'|!|$)/g, (_: string, v0: string, v: string, v1 = '') => `${v0}-${config.strictVariable ? '[' : ''}${v.replace(/\s*/g, '')}${config.strictVariable ? ']' : ''}${v1}`],
    config.strictHyphen
      ? [/([A-Za-z]+)-(\#[^\s']+)(\s|'|!|$)/g, (_: string, v0: string, v1: string, v2: string) => `${v0}-${config.strictVariable ? '[' : ''}${v1}${config.strictVariable ? ']' : ''}${v2}`]
      : [/([A-Za-z]+)-?(\#[^\s']+)(\s|'|!|$)/g, (_: string, v0: string, v1: string, v2: string) => `${v0}-${config.strictVariable ? '[' : ''}${v1}${config.strictVariable ? ']' : ''}${v2}`],
    config.strictHyphen
      ? [/([A-Za-z]+)-([0-9]+)((?:px)|(?:vw)|(?:vh)|(?:rem)|(?:em)|(?:%))(\s|'|!|$)/g, (_: string, v0: string, v1: string, v2 = '', v3 = '') => config.strictVariable ? `${v0}-[${v1}${v2}]${v3}` : `-${v1}${v2}${v3}`]
      : [/([A-Za-z]+)-?([0-9]+)((?:px)|(?:vw)|(?:vh)|(?:rem)|(?:em)|(?:%))(\s|'|!|$)/g, (_: string, v0: string, v1: string, v2 = '', v3 = '') => config.strictVariable ? `${v0}-[${v1}${v2}]${v3}` : `-${v1}${v2}${v3}`],
    [config.strictHyphen
...
const attrRules = config.strictHyphen
